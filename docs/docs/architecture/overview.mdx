...
The user authenticates via Google → Keycloak; Keycloak redirects the browser (through EnterpriseClaw's shared ALB + Istio Gateway) to `/auth/callback?code=&state=`. The broker then runs three internal steps:
...
sequenceDiagram
    ...
    participant ALB as EnterpriseClaw ALB + Istio GW
